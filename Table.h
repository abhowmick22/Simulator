...
KVTemplate class table_t {
  ...
  public:
    ...
    virtual entry insert(key_t key, value_t value, policy_value_t pval = 0) {
      uint32 index;
    ...
    virtual entry read(key_t key, policy_value_t pval = 0) {
      uint32 index;
    ...
    virtual entry update(key_t key, value_t value, policy_value_t pval = 0) {
      uint32 index;
    ...
    virtual entry silentupdate(key_t key, policy_value_t pval = 0) {
      uint32 index;
      // check if the key is present
      if ((index = SearchForKey(key)) == _size)
        return entry();
    ...
    virtual entry invalidate(key_t key) {
      ...
      UpdateReplacementPolicy(index, T_INVALIDATE, 0);
      entry evicted = _table[index];
